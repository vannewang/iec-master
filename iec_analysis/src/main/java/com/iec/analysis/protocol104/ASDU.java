...
public class ASDU {
     ...
     * @throws Exception
     */
    public static String Infoanalysis(int[] info, int i, int j, int k) {
        String s = "";
        switch (i) {
            case 1:
     ...
     * @return
     */
    public static Map<String, String> variTureDete(int b) {
        Map<String, String> map = new HashMap<>(2);
        String vvString = Util.toHexString(b);
        if ((b & 0x80) == 128) {
            map.put("order", "1");
            System.out.println("SQ=1 信息元素地址顺序     ");
        } else if ((b & 0x80) == 0) {
            map.put("order", "0");
            System.out.println("SQ=0  信息元素地址非顺序     ");
        }
        //信息元素个数
        Integer i = b & 0x7f;
        map.put("number", i.toString());
        return map;
    }
